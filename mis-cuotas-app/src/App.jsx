...
// Adaptador de almacenamiento: usa window.storage si existe (entorno Claude Artifacts),
// y si no, cae a localStorage del navegador (para correr la app fuera de Claude, ej. en GitHub Pages/Vercel).
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // No hay datos guardados todavía: se parte con listas vacías.
      } finally {
        if (!cancelled) {
          setLoading(false);
          setHasLoaded(true);
  ...
  useEffect(() => {
    ...
    async function save() {
      try {
        await storage.set(STORAGE_KEY, JSON.stringify({ items, pagos, ocultos }), false);
        setSaveError(false);
      } catch (e) {
        setSaveError(true);
      }
        ...
        <div className="px-5">
          ...
          {saveError && (
            <p className="text-xs text-red-500 mt-3 text-center">
              No se pudieron guardar los últimos cambios. Revisa tu conexión.
            </p>
          )}
        </div>
        ...
        {showSettings && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl w-full max-w-sm p-6">
              ...
              <p className="text-sm text-stone-500 mb-5">
                Tus gastos y pagos se guardan automáticamente y quedan disponibles la próxima vez que abras la app.
              </p>
